...
<script type="text/javascript" src="${webPath}jsp/systemMng/user/js/user.js?v=2017030902"></script>
	  ...
	  <div class="landtitle">
	    ...
	    <div class="mb10 mt10 searchVo">
				<form class=" form-inline form-inline-sm">
					<!-- 点击新建的时候是展示另外一个界面 -->
					<a role="button" class="btn btn-primary mr8" href="${webPath}systemMng/user/toCreateUser.do"  >新增用户</a>
<!-- 					<a href="javascript:void(0);" onclick="delConfirm('all')" class="btn btn-default mr18 hui"><i class="ico-del-link icon-bg">批量删除</i></a> -->
					<div class="form-group form-group-sm">
						<input class="form-control input-large" id="searchTxt" param="search.content" placeholder="按登录账号查询">
					</div>
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable caozuo  mb20 table">
					<tr>
							...
							<th class="text-left" width="30%">登录账号</th>
							<th class="text-left" width="30%">用户名</th>
							<th  class="text-center" width="20%">身份</th>
				   ...
				   {{#obj}}
				   	<tr>
 						...
 						{{userName}}
						  ...
						  <td class="text-center"><span class="hui">{{typeName}}</span></td>
					       <td><p class="clearfix text-right  pr8">
					   		       <a class="ico-del icon-caozuo f0 fr mr10" href="javascript:void(0);" onclick="delConfirm('single','{{userKey}}','{{type}}');" title="删除" >删除</a>
					   		       <a class="ico-edit icon-caozuo f0 fr mr10" href="javascript:void(0);"onclick="updateConfirm('{{userKey}}');" title="密码重置" >密码重置</a>
				   		    </p>
				   		   </td>
					  </tr>
				   {{/obj}}
...
<div class="pop-up w360  bg-hui01" id="alertdiv8">
	<div class="pt30 pb30 text-center">
		<p id="p_result7" class="hei01 f16 mt16">是否确定把密码重置为1?</p>
	</div>
	<p class="text-center mb40">
		<a href="javascript:void(0);" id="ok8" class="f20 bai bg-lv-cur display-ib lh38 h38 bd-lv br4 text-center w158">确 定</a>
		<span><a href="javascript:void(0);"onclick="$('#alertdiv8').CloseDiv()" class="f20 hui display-ib w158 btn-bg02 bd-hui01 lh38 h38 br4 text-center ml6">取消</a></span>
	</p>
</div>
 <script type="text/javascript">
		...
		//点击删除时触发此方法，此方法主要给提示框绑定删除方法和打开提示框
		function delConfirm(type,id,typeName){
			$('#ok').attr("onclick","delete_all('"+type+"','"+id+"','"+typeName+"')");
			$('#alertdiv7').OpenDiv();
		}
		//删除执行方法
		function delete_all(type,id,typeName){
			var ids='';
			if(typeName=='1'){
				layer.alert("管理员不可删除");
				return;
			...
			//执行删除方法
			var url="${webPath}systemMng/user/deleteUser.do?key="+ids;
...
// 		}
		...
		function updateConfirm(id){
        	 $('#ok8').attr("onclick","update('"+id+"')");
			$('#alertdiv8').OpenDiv();
		}
         
         function update(userKey){
        	 var url="${webPath}systemMng/user/updateUserPassword.do?userKey="+userKey+"&password=1";
 			$.post(url,function(json){
 				if(json.success){
 					$('#alertdiv8').CloseDiv();//关闭提示框
 					 layer.alert('密码重置成功！');
 				}
 			});
         }
