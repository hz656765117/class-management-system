...
<script type="text/javascript" src="${webPath}jsp/studyMng/evaluate/js/evaluate.js?v=2017030902"></script>
	  ...
	  <div class="landtitle">
	    ...
	    <div class="mb10 mt10 searchVo">
				<form class=" form-inline form-inline-sm">
					<!-- 点击新建的时候是展示另外一个界面 -->
					<a role="button" class="btn btn-primary mr8" href="${webPath}studyMng/evaluate/toCreateEvaluate.do"  >新建</a>
...
<!-- 					<a role="button" class="btn btn-success btn-primary">查询</a> -->
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable caozuo  mb20 table">
					<tr>
							...
							<th class="text-left" width="32%">评教活动</th>
 						...
 						{{title}}
					      ...
					      {{statusStr}}
					       ...
					       <td><p class="clearfix text-right  pr8">
					   		       <a class="ico-del icon-caozuo f0 fr mr10" href="javascript:void(0);" onclick="delConfirm('single','{{evaluateKey}}');" title="删除" >删除</a>
						   		   <a class="ico-edit icon-caozuo f0 fr mr10" href="javascript:void(0);"onclick="getById('{{evaluateKey}}','{{status}}');" title="编辑" >编辑</a>
				   		    </p>
				   		   </td>
					  </tr>
				   {{/obj}}
...
<!-- 选择判断是否的选择面板 -->
 ...
 <script type="text/javascript">
		...
		function delete_all(type,id){
			...
			//执行删除方法
			var url="${webPath}studyMng/evaluate/deleteEvaluate.do?keyArr="+ids;
		...
		function getById (id,status){
			if(status=='1'){
				alert("该信息已经发布，不能再进行修改！");
				return;
			}
			var url="${webPath}studyMng/evaluate/toCreateEvaluate.do?evaluateKey="+id;
			location = url;
		}
 </script>