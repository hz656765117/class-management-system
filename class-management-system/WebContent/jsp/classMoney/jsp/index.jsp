...
<script type="text/javascript" src="${webPath}jsp/classMoney/js/money.js?v=2017030902"></script>
...
<%-- 				<a class="blue f12" style="margin-left:18px;" href="JavaScript:window.open('${webPath}/helps/helps_list_80.html')"><img src="${webPath}images/admin/grayquestion.png" class="ver-3 mr5">使用帮助</a> --%>
	    ...
	    <div class="mb10 mt10 searchVo">
				<form class=" form-inline form-inline-sm">
					<!-- 点击新建的时候是展示另外一个界面 -->
					<!-- 只有教师才有新建的权限,如果是教师和管理员，就显示所有班级的数据，如果是学生就只显示这个学生的数据 -->
					<c:if test="${type=='2'}">
						<a role="button" class="btn btn-primary mr8" href="${webPath}classMoney/toCreateMoney.do"  >新建</a>
					</c:if>
...
<!-- 					<a role="button" class="btn btn-success btn-primary">查询</a> -->
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable caozuo  mb20 table">
					<tr>
							...
							<th class="text-left" width="10%">类型</th>
							<th  class="text-left" width="10%">金额</th>
							<th  class="text-left" width="40%">支出/来源</th>
							<th  class="text-left" width="15%">日期</th>
							<c:if test="${type=='2'}">
			   ...
			   <textarea id="template" class="template" style="display: none">
				   {{#obj}}
				   	<tr>
					      <td class="text-center"><input type="checkbox" name="box" id="box" value="{{classMoneyKey}}" /></td>
					      <td class="text-left">
 						{{className}}
						</td>
					     ...
					     <c:if test="${type=='2'}">
						       <td><p class="clearfix text-right  pr8">
						   		       <a class="ico-del icon-caozuo f0 fr mr10" href="javascript:void(0);" onclick="delConfirm('single','{{classMoneyKey}}');" title="删除" >删除</a>
						   		       <a class="ico-edit icon-caozuo f0 fr mr10" href="javascript:void(0);"onclick="getById('{{classMoneyKey}}');" title="编辑" >编辑</a>
					   		    </p>
					   		   </td>
				   		   </c:if>
					  </tr>
...
<!-- 选择判断是否的选择面板 -->
 ...
 <script type="text/javascript">
		...
		function delete_all(type,id){
			...
			//执行删除方法
			var url="${webPath}classMoney/deleteClassMoney.do?keyArr="+ids;
		...
		//获取修改面板		
		function getById (id){
			var url="${webPath}classMoney/toUpdateClassMoney.do?classMoneyKey="+id;
			location = url;
		}
 </script>