    	...
    	<div class="">
	        <div class=" pos-rel mt14 pb10 ml20 mr20 bb-hui02 text-center clearfix"> <a href="javascript:void(0);" onclick="back()" class="w78 bd-hui02 display-ib text-center h28 lh26 f14 btn-hui01 pos-tp0"><i class="icon-bg icon-back">返回</i></a>
	        	<c:if test="${not empty course.id}">
		          	<h2 id="surveytitle" class="f18 hei01 nobold lh30">修改课程信息</h2>
		        </c:if>
      		...
      		<div class="m0a" style="margin-top:42px;width:480px">
		        <div class="f14 lh30  hei01  mt20 clearfix">
		          ...
		          <input id="name" type="text" value="${course.name}" placeholder="请输入课程" class="lh28 h28 w322 pl10 bd-hui02 fl ml10">
		        </div>
		        <div class="f14 lh30  hei01  mt20 clearfix">
		          <label class="f14 fl hei01 w80 fl lh30 text-right">描述：</label>
		          <textarea id="comments" class="lh28 h56 w322 pl10 bd-hui02 fl ml10">${course.comments}</textarea>
		        </div>
...
String.prototype.replaceAll = function(s1,s2) { 
 ...
 //返回
function back(){
	window.location.href=webPath+"studyMng/course/index.do";
}
...
function createAcademy(){
	...
	parameters['comments']=$('#comments').val();
	parameters['id']=$('#id').val();
	
	var url = "${webPath}studyMng/course/insertCourse.do";
	if ($.trim($('#id').val())!='') {
		url = "${webPath}studyMng/course/updateCourse.do";
	}
	...
	$.ajax({
		...
		success : function(data) {
			if(data.success!=null&&data.success){
				location.href = "${webPath}studyMng/course/index.do";
			}else{
				alert(data.msg);
			}
		},
