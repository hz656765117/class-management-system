...
(function ($) {
	...
	var defaults = { // 插件默认值
		...
		viewrange: '全体人员', // 可选范围tab; string || null
	...
	function zTreeInit() {
		...
		var departSetting = {
			async: {
				enable: true,
				url: ctx + "/contactPlugin/listClass.do?appid=TXL",
				autoParam: ["id=pid"],
				dataFilter: filter,
				type: 'get'
			},
		...
		function queryPerson() {
			...
			$.ajax({
				url: ctx + '/contactPlugin/listPersonByClass.do',
				data: param,
				async: false,
				success: function (data) {
					...
					for (var i in persons) {
						...
						var $li = $(''
							+ '<li id="person_' + person.studentKey + '">'
