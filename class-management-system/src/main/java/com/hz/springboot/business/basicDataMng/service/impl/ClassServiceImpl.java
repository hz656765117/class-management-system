...
import com.hz.springboot.business.basicDataMng.base.dao.ClassPOMapper;
...
public class ClassServiceImpl implements ClassService {
	...
	@Override
	public List<ClassPO> classListByMajorKey(String majorKey) {
		ClassPOExample example = new ClassPOExample();
		//创建方法，添加搜索条件
		example.createCriteria().andMajorKeyEqual(majorKey);
		List<ClassPO>  list = classPOMapper.selectByExample(example);
		return list;
	}

	@Override
	public List<ClassPO> listAllClass() {
		ClassPOExample example = new ClassPOExample();
		List<ClassPO>  list = classPOMapper.selectByExample(example);
		return list;
	}
	...
	@Override
	public List<ClassPO> selectClassMoneyList(String content) {
		ClassPOExample example = new ClassPOExample();
		//创建方法，添加搜索条件
//		example.createCriteria().andListLikeTo(className);
		List<ClassPO>  list = classPOMapper.selectClassMoneyByExample(example);
		return list;
	}
